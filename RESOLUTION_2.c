...
bool backtracking(tGrille grille, int numeroCase) {
  ...
  if (numeroCase == TAILLE * TAILLE) {
    ...
    if (grille[ligne][colonne].valeur != 0) {
      ...
      for (int indCand = 0; indCand < grille[ligne][colonne].nbCandidats; indCand++) {
        if (possible(grille, ligne, colonne, grille[ligne][colonne].candidats[indCand])) {
          // Si la valeur est autorisée on l'inscrit
          // dans la case...
          grille[ligne][colonne].valeur = grille[ligne][colonne].candidats[indCand];
