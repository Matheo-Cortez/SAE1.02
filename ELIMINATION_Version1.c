...
int singletonNu(tGrille grille, bool *avancer) {
  ...
  for (lig = 0; lig < TAILLE; lig++) {
    for (col = 0; col < TAILLE; col++) {
      if (grille[lig][col].valeur == 0) {
        if (grille[lig][col].nbCandidats == 1) {
          grille[lig][col].valeur = grille[lig][col].candidats[0];
          *avancer = true;
...
int singletonCache(tGrille grille, bool *avancer) {
  ...
  for (lig = 0; lig < TAILLE; lig++) {
    for (val = 1; val <= TAILLE; val++) {
      ...
      while (col < TAILLE && apparait < 2) {
        if (grille[lig][col].valeur == 0) {
          indCandidat = 0;
          // Lis tous les candidats d'une case pour en trouver un = à valeur
          while (indCandidat < grille[lig][col].nbCandidats && apparait < 2) {
            if (grille[lig][col].candidats[indCandidat] == val) {

              // Si la valeur = candidat sur une autre colonne donc qu'il y a 2
      ...
      // dans la case
      if (apparait == 1 && indColtempo <= TAILLE) {
    ...
    for (val = 1; val <= TAILLE; val++) {
      ...
      while (lig < TAILLE && apparait < 2) {
        if (grille[lig][col].valeur == 0) {
          ...
          while (indCandidat < grille[lig][col].nbCandidats && apparait < 2) {
            if (grille[lig][col].candidats[indCandidat] == val) {
              // Si la valeur = candidat sur une autre ligne donc qu'il y a 2
              // fois le candidat dans la colonne
      ...
      // Si à la fin de la ligne on a trouvé un candidat unique alors on le met
      // dans la case
      if (apparait == 1 && indLigTempo <= TAILLE) {
        grille[indLigTempo][indColtempo].valeur = val;
        *avancer = true;                   // Pour la boucle du main
        casesRemplies = casesRemplies + 1; // Pour le nombre de cases remplies
      }
  ...
  for (int ligBloc = 0; ligBloc < TAILLE;
       ligBloc += N) { // boucle pour les lignes du bloc
    for (int colBloc = 0; colBloc < TAILLE;
         colBloc += N) { // boucle pour les colonnes du bloc
      for (val = 1; val <= TAILLE; val++) {
        apparait = 0;    // reset
        indLigTempo = 0; // pour stocker la ligne du bloc
        indColtempo = 0; // pour stocker la colonne du bloc
        for (lig = ligBloc; lig < ligBloc + N;
             lig++) { // Parcourt le bloc pour chercher la valeur
          for (col = colBloc; col < colBloc + N; col++) { // Pareil
            if (grille[lig][col].valeur == 0) {
              int indCandidat = 0;
              while (indCandidat < grille[lig][col].nbCandidats &&
                     apparait < 2) {
                if (grille[lig][col].candidats[indCandidat] ==
                    val) { // si le candidat est égal à la valeur
                  if (apparait == 1 &&
                      (col != indColtempo || lig != indLigTempo)) {
                    apparait = apparait + 1; // sort de toutes les boucles
                  }
        ...
        if (apparait == 1 && indLigTempo < ligBloc + N &&
            indColtempo < colBloc + N) { // Si la valeur n'apparaît qu'une seule
                                         // fois dans le bloc, la placer
          grille[indLigTempo][indColtempo].valeur = val;
          *avancer = true;
          casesRemplies++;
        }
...
void afficherStats(int nbCasesVidesFinale, int nbCasesVidesInitial,
                   int nbCandidatsFinale, int nbCandidatsInitial,
                   char nomFichier[30]) {
